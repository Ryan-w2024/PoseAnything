...
from accelerate.utils import DistributedDataParallelKwargs,DDPCommunicationHookType
...
class ModelLogger:
    ...
    def on_step_end(self, accelerator, model, save_steps=None, average_loss=None):
        print("AVERAGE_LOSS:", average_loss)
        if save_steps is not None and self.num_steps % save_steps == 0:
            self.save_model(accelerator, model, f"step-{self.num_steps}.safetensors", average_loss)



    def on_epoch_end(self, accelerator, model, epoch_id, average_loss):
        ...
        if accelerator.is_main_process:
            ...
            import csv  # 写入average loss
            metric_path = os.path.join(self.output_path, "metric.csv")
            metric_data = {"model_file": f"epoch-{epoch_id}.safetensors", "loss_mean": average_loss}
            file_exists = os.path.exists(metric_path)
    ...
    def save_model(self, accelerator, model, file_name, loss_mean):
        accelerator.wait_for_everyone()
        if accelerator.is_main_process:
            state_dict = accelerator.get_state_dict(model)
            state_dict = accelerator.unwrap_model(model).export_trainable_state_dict(state_dict, remove_prefix=self.remove_prefix_in_ckpt)
            ...
            import csv  # 写入average loss
            metric_path = os.path.join(self.output_path, "metric.csv")
            metric_data = {"model_file": file_name,"loss_mean": loss_mean}
            file_exists = os.path.exists(metric_path)
            with open(metric_path, "a", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(f, fieldnames=metric_data.keys())
                if not file_exists:
    ...
    for epoch_id in range(num_epochs):
        ...
        for data in tqdm(dataloader):
            with accelerator.accumulate(model):
                ...
                optimizer.step()
                average_loss = epoch_loss_sum / step_sum
                model_logger.on_step_end(accelerator, model, save_steps, average_loss)
                scheduler.step()
        if save_steps is None:
            average_loss = epoch_loss_sum / step_sum
            model_logger.on_epoch_end(accelerator, model, epoch_id, average_loss)
...
def wan_parser():
    ...
    # added
    parser.add_argument("--steps_per_epoch", type=int, default=2500, help="steps per epoch.")
    parser.add_argument("--human_dataset_base_path", type=str, default="", required=False, help="Base path of the human dataset.")
    parser.add_argument("--human_dataset_metadata_path", type=str, default=None,
                        help="Path to the metadata file of the human dataset.")
    parser.add_argument("--non_human_dataset_base_path", type=str, default="", required=False,
                        help="Base path of the non_human dataset.")
    parser.add_argument("--non_human_dataset_metadata_path", type=str, default=None,
                        help="Path to the metadata file of the non human dataset.")
    return parser
