...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        first_skeleton = condition_images[0].copy()
        condition_images_neg = condition_images.copy()
        if pose_cfg:
            for i in condition_index:
                condition_images_neg[i]=first_skeleton
                ...
                for f_i in [l_i * 4 + 1, l_i * 4 + 2, l_i * 4 + 3, l_i * 4 + 4]:
                    ...
                    for f_m_i, first_mask in enumerate(first_masks):
                        ...
                        for i, cur_mask in enumerate(cur_masks):
                            ...
                            if selected_attn_weights.numel() == 0:
                                continue
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            noise_pred_posi = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep,
                                            union_mask=union_mask, first_masks=first_masks, progress_id=progress_id)
            if cfg_scale != 1.0:
                if cfg_merge:
                    noise_pred_posi, noise_pred_nega = noise_pred_posi.chunk(2, dim=0)
                else:
    ...
    **kwargs,
    ...
    if condition_emb is not None:
        if False:
            ...
            x = x + dit.condition_proj(condition_emb.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        else:
            x = torch.cat([x, condition_emb], dim=1)
