...
from ..models.wan_video_dit_mlp import WanModel, RMSNorm, sinusoidal_embedding_1d
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.units = [
            ...
            WanVideoUnit_InputSkeletonEmbedder(),
    ...
    def training_loss(self, mlp_condition=False, view_condition=False, **inputs):
        ...
        noise_pred = self.model_fn(**inputs, mlp_condtion=mlp_condition, view_condition=view_condition, timestep=timestep)
    ...
    def draw_hollow_rect(self, img, top, left, rect_h, rect_w):
        # img: [3, h, w]
        img[:, top, left:left + rect_w] = 1  # 上边
        img[:, top + rect_h - 1, left:left + rect_w] = 1  # 下边
        img[:, top:top + rect_h, left] = 1  # 左边
        img[:, top:top + rect_h, left + rect_w - 1] = 1  # 右边

    def find_rect_top_pos(self, condition_images):
        # 仅处理第一帧
        mask_white = (condition_images[0] == 1).any(dim=0)  # [h, w]
        white_rows = mask_white.any(dim=1).nonzero(as_tuple=True)[0]
        h = condition_images.shape[2]
        if len(white_rows) == 0:
            return 0  # 没有骨架，默认最上方
        top_white = white_rows[0].item()
        bottom_white = white_rows[-1].item()
        dist_to_top = top_white
        dist_to_bottom = h - bottom_white - 1
        if dist_to_top > dist_to_bottom:
            rect_top = top_white // 2
        else:
            rect_top = (bottom_white + h - 1) // 2
        return rect_top

    def find_rect_top_pos(self, condition_images):
        # 仅处理第一帧
        mask_white = (condition_images[0] > 0.9).any(dim=0)  # [h, w]
        white_rows = mask_white.any(dim=1).nonzero(as_tuple=True)[0]
        white_cols = mask_white.any(dim=0).nonzero(as_tuple=True)[0]
        h = condition_images.shape[2]
        w = condition_images.shape[3]

        if len(white_rows) == 0:
            return 0, 0  # 没有骨架，默认左上角

        top_white = white_rows[0].item()
        bottom_white = white_rows[-1].item()
        left_white = white_cols[0].item()
        right_white = white_cols[-1].item()
        ...
        dist_to_left = left_white
        dist_to_right = w - right_white - 1
        if dist_to_left > dist_to_right:
            rect_left = left_white // 2
        else:
            rect_left = (right_white + w - 1) // 2

        return rect_top, rect_left

    def motion_v1(self, condition_images, condition_index, direction="left"):
        '''
        condition_images: (f, 3, h, w) tensor
        condition_index: list of indices
        direction: 运动方向，可选 "left", "right", "up", "down"
        '''
        f, c, h, w = condition_images.shape
        device = condition_images.device
        dtype = condition_images.dtype

        condition_images_new = torch.full((f, c, h, w), -1, dtype=dtype, device=device)  # 全黑
        condition_images_new[0] = condition_images[0].clone()  # 第一帧复制

        rect_h = h // 20
        rect_w = w // 20
        move_step = (w // 20 * 19) // 81 * 2  # 防止除零

        # 第一帧画矩形
        rect_top, rect_left = self.find_rect_top_pos(condition_images)

        # 根据方向设置初始位置
        if direction == "left":
            rect_left = w - 1 - rect_w  # 从右侧开始向左移动
        elif direction == "right":
            rect_left = 0  # 从左侧开始向右移动
        elif direction == "up":
            rect_top = h - 1 - rect_h  # 从底部开始向上移动
        elif direction == "down":
            rect_top = 0  # 从顶部开始向下移动

        self.draw_hollow_rect(condition_images_new[0], rect_top, rect_left, rect_h, rect_w)

        # 后续帧
        for i in range(1, f):
            # 全为0则不画
            if direction == "left":
                left = rect_left - move_step * i
                left = max(left, 0)
                top = rect_top
            elif direction == "right":
                left = rect_left + move_step * i
                left = min(left, w - rect_w)
                top = rect_top
            elif direction == "up":
                top = rect_top - move_step * i
                top = max(top, 0)
                left = rect_left
            elif direction == "down":
                top = rect_top + move_step * i
                top = min(top, h - rect_h)
                left = rect_left

            self.draw_hollow_rect(condition_images_new[i], top, left, rect_h, rect_w)

        return condition_images_new

    def shift_bone_mask_left(self, bone_mask, shift):
        # bone_mask: [3, h, w], shift: int
        # 向左平移，丢弃左侧，右侧补零
        shifted = torch.zeros_like(bone_mask)
        if shift == 0:
            shifted = bone_mask.clone()
        elif shift < bone_mask.shape[2]:
            shifted[:, :, :bone_mask.shape[2] - shift] = bone_mask[:, :, shift:]
        # 如果shift超出宽度，则全为0
        return shifted

    def motion_v2(self, condition_images, condition_index):
        '''
        condition_images: (f, 3, h, w) tensor
        condition_index: list of indices
        '''
        ...
        _condition_images_new = torch.full((f, c, h, w), -1, dtype=dtype, device=device)  # 全黑
        _condition_images_new[0] = condition_images[0].clone()  # 第一帧复制

        # # 规则2：指定帧全黑
        # for i in condition_index:
        #     _condition_images_new[i] = -1

        # 规则3：找出第一帧白色骨架最左边距
        mask_white = (condition_images[0] == 1).any(dim=0)  # [h, w]
        white_cols = mask_white.any(dim=0).nonzero(as_tuple=True)[0]  # 所有有白色的列索引
        if len(white_cols) == 0:
            return condition_images #不需要平移
        else:
            left_white = white_cols[0].item()
            d = left_white

        bone_mask = (condition_images[0] == 1).float()  # [3, h, w]骨架mask
        for i in range(f):
            # 跳过全黑帧
            # if torch.all(_condition_images_new[i] == -1):
            #     continue
            if torch.all(condition_images[i] == -1):
                shift = d // 81 * i * 2 if 81 != 0 else 0
                shifted_bone = self.shift_bone_mask_left(bone_mask, shift)
                # 将平移骨架mask的白色部分赋值为1，覆盖原图
                _condition_images_new[i][shifted_bone == 1] = 1
                # 其他像素保持原样
        return _condition_images_new

    def motion_v3(self, condition_images, condition_index):
        '''
        condition_images: (f, 3, h, w) tensor
        condition_index: list of indices
        '''
        ...
        for i in range(1, f):
            if i in condition_index:
                condition_images_new[i] = 0  # 置零

        rect_h = h // 20
        rect_w = w // 20

        # 速率定义（每帧移动的距离，可以自行调整）
        move_step = max(1, w // 81)

        rect_top = self.find_rect_top_pos(condition_images)
        # 两个矩形的初始位置
        rect_left1_init = w // 2
        rect_left2_init = w - rect_w

        # 第一帧画两个矩形
        self.draw_hollow_rect(condition_images_new[0], rect_top, rect_left1_init, rect_h, rect_w)
        self.draw_hollow_rect(condition_images_new[0], rect_top, rect_left2_init, rect_h, rect_w)

        # 后续帧
        for i in range(1, f):
            if torch.all(condition_images_new[i] == 0):
                continue
            if torch.all(condition_images_new[i] == -1):
                # 计算矩形新的位置
                left1 = (rect_left1_init - move_step * i) % (w + 1)
                left2 = (rect_left2_init - move_step * i) % (w + 1)
                # 如果矩形超过边界，保证不会越界
                left1 = min(left1, w - rect_w)
                left2 = min(left2, w - rect_w)
                self.draw_hollow_rect(condition_images_new[i], rect_top, left1, rect_h, rect_w)
                self.draw_hollow_rect(condition_images_new[i], rect_top, left2, rect_h, rect_w)

        return condition_images_new

    def motion_v5(self, img_path, h, w):
        # 读取并resize图片
        ...
        # 判断白色像素，设定阈值
        # 允许容差, 例如 (250, 250, 250) 以上都认为是白色
        white_mask = np.all(img_arr >= 253, axis=2)  # [h, w]

        # 构造 first_background: shape [3, h, w]
        first_background = np.full((3, h, w), -1.0, dtype=np.float32)
        for c in range(3):
            channel = img_arr[:, :, c]
            first_background[c][white_mask] = 1.0

        first_background = torch.from_numpy(first_background)  # [3, h, w]

        # 生成81帧张量
        F = 81
        frames = torch.full((F, 3, h, w), -1.0, dtype=torch.float32)
        frames[0] = first_background.clone()
        # 白色区域mask
        white_mask = (first_background == 1.0)
        d = w // 4  # 平移距离

        for i in range(1, F):
            shifted = torch.full((3, h, w), -1.0, dtype=torch.float32)
            for c in range(3):
                mask = white_mask[c]
                if mask.sum() > 0:
                    # 左移 d*i 个像素
                    ...
                    if shift < w:
                        # 剩余部分
                        shifted[c, :, :w - shift][mask[:, shift:]] = 1.0
                    # 否则全部移出画面
                    # 如果全部移出画面, remain为0, shifted全为-1
                # 若mask.sum==0, shifted全为-1
            # 检查是否还有白色像素
            if (shifted == 1.0).sum() == 0:
                shifted[:] = -1.0
            frames[i] = shifted
    ...
    # 调用 model(data)
    def images_to_tensor(self, condition_images):
        if not condition_images:
            raise ValueError("输入列表 condition_images 为空")

        tensor_list = []
        for img in condition_images:
            img_np = np.array(img, dtype=np.float32)
            if len(img_np.shape) == 2:
                img_np = img_np[..., np.newaxis]  # 灰度图添加通道维度]
    ...
    def tensor_to_images(self, batch_tensor):
        """
        将形状为 [f, c, h, w]、像素值 [-1,1] 的张量转换回 PIL Image 列表（像素值 [0,255]）
        :param batch_tensor: 输入张量，形状 [f, c, h, w]，dtype=float32
        :return: 列表，每个元素是 PIL.Image 对象
        """
        # 1. 检查输入张量形状
        if len(batch_tensor.shape) != 4:
            raise ValueError(f"输入张量需为 4 维 [f, c, h, w]，当前形状：{batch_tensor.shape}")
        ...
        for i in range(batch_tensor.shape[0]):
            ...
            img_np = img_tensor.permute(1, 2, 0).cpu().detach().numpy()  # 移到 CPU 并转为 numpy
            img_np = (img_np + 1.0) / 2.0 * 255.0

            img_np = np.clip(img_np, 0, 255)
            img_np = img_np.astype(np.uint8)
            if img_np.shape[-1] == 1:
                img_np = img_np.squeeze(-1)  # [h, w, 1] → [h, w]
                img = Image.fromarray(img_np, mode='L')
            else:
                # 多通道图（RGB）：直接转换，默认 'RGB' 模式
    ...
    def __call__(
        ...
        inputs_posi = {
            ...
            "condition_images": condition_images, "condition_index": condition_index,
        }
        inputs_nega = {
            "negative_prompt": negative_prompt,
            "tea_cache_l1_thresh": tea_cache_l1_thresh, "tea_cache_model_id": tea_cache_model_id,"num_inference_steps": num_inference_steps,
            "condition_images_neg": condition_images_neg, "condition_index": condition_index,
        }
        inputs_shared = {
            ...
            "camera_control_direction": camera_control_direction, "camera_control_speed": camera_control_speed,
            "camera_control_origin": camera_control_origin,
            "vace_video": vace_video, "vace_video_mask": vace_video_mask,
            "vace_reference_image": vace_reference_image, "vace_scale": vace_scale,
            ...
            "audio_embeds": audio_embeds, "s2v_pose_latents": s2v_pose_latents, "motion_video": motion_video,
        }
        ...
        if binary_masks is not None:
            ...
            for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
                ...
                if timestep.item() < switch_DiT_boundary * self.scheduler.num_train_timesteps and self.dit2 is not None and not \
                models["dit"] is self.dit2:
                    self.load_models_to_device(self.in_iteration_models_2)
                    models["dit"] = self.dit2
                timestep = timestep.unsqueeze(0).to(dtype=self.torch_dtype, device=self.device)
                #只计算注意力权重
                if progress_id == 5:
                    attn_weights = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep, check=True,
                                                 file_name="111", progress_id=progress_id)
                    break

                noise_pred_posi = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep, progress_id=progress_id)
                if cfg_scale != 1.0 and file_name == "":
                    if cfg_merge:
                        noise_pred_posi, noise_pred_nega = noise_pred_posi.chunk(2, dim=0)
                    else:
                        noise_pred_nega = self.model_fn(**models, **inputs_shared, **inputs_nega, timestep=timestep,
                                                        progress_id=progress_id, skip=False)
                    noise_pred = noise_pred_nega + cfg_scale * (noise_pred_posi - noise_pred_nega)
                else:
                    noise_pred = noise_pred_posi

                inputs_shared["latents"] = self.scheduler.step(noise_pred, self.scheduler.timesteps[progress_id],
        ...
        #用attn_weights匹配
        print("attn_weights shape", attn_weights.shape) #[1, 24, 364, 7644]
        _, _, _, h, w = inputs_shared["latents"].shape
        b, num_heads, seq_length, total_length = attn_weights.shape
        #seq_length, num_heads, total_length = attn_weights.shape
        #attn_weights = torch.sum(attn_weights, dim=0)
        attn_weights = torch.sum(attn_weights, dim=1)
        #attn_weights = attn_weights.unsqueeze(0)
        print("attention_weights shape after sum", attn_weights.shape)

        # 匹配masks
        first_masks = binary_masks[0]  # 第一帧的mask
        # print("h",h,"w", w) # h 104 w 58
        union_mask = torch.zeros([20, len(first_masks), h // 2, w // 2], dtype=torch.bool)

        for l_i in range(20):
            attn_weight = attn_weights[:, :, l_i * seq_length:(l_i + 1) * seq_length]
            for f_i in [l_i * 4 + 1, l_i * 4 + 2, l_i * 4 + 3, l_i * 4 + 4]:
                if f_i not in condition_index:
                    continue
                f_index = condition_index.index(f_i)
                cur_masks = binary_masks[f_index]
                if not cur_masks:
                    continue
                for f_m_i, first_mask in enumerate(first_masks):
                    max_attn_weights_mean = -float('inf')
                    max_attn_index = -1
                    first_mask_flat = torch.from_numpy(first_mask).to(torch.bool).reshape(-1)
                    #first_mask_flat = first_mask.to(torch.bool).reshape(-1)

                    # 与所有后续帧进行配对
                    for i, cur_mask in enumerate(cur_masks):
                        cur_mask_flat = torch.from_numpy(cur_mask).to(torch.bool).reshape(-1)
                        #cur_mask_flat = cur_mask.to(torch.bool).reshape(-1)
                        selected_attn_weights = attn_weight[:, first_mask_flat, :][:, :, cur_mask_flat]
                        if selected_attn_weights.numel() == 0:  # numel()：统计张量元素总数，0表示空
                            continue
                        attn_weights_mean = selected_attn_weights.mean()

                        if attn_weights_mean > max_attn_weights_mean:
                            max_attn_weights_mean = attn_weights_mean
                            max_attn_index = i

                    if max_attn_index == -1 or max_attn_index >= len(cur_masks):
                        continue

                    cur_mask_tensor = torch.tensor(cur_masks[max_attn_index], device=union_mask.device)
                    union_mask[l_i - 1][f_m_i] = torch.logical_or(union_mask[l_i][f_m_i], cur_mask_tensor)
        del attn_weights
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            noise_pred_posi = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep,
                                            union_mask=union_mask, first_masks=first_masks,
                                            file_name=file_name, progress_id=progress_id)
            if cfg_scale != 1.0 and file_name == "":
                if cfg_merge:
                    noise_pred_posi, noise_pred_nega = noise_pred_posi.chunk(2, dim=0)
                else:
...
class WanVideoUnit_InputSkeletonEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            seperate_cfg=True,
            input_params_posi={"condition_images": "condition_images", "tile_size": "tile_size", "tile_stride": "tile_stride", "tiled": "tiled"},
            input_params_nega={"condition_images": "condition_images_neg", "tile_size": "tile_size", "tile_stride": "tile_stride", "tiled": "tiled"},
            #input_params=("condition_images", "tiled", "tile_size", "tile_stride"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, condition_images, tiled, tile_size, tile_stride):
        if condition_images is None:
            return {"condition_emb": None}
        pipe.load_models_to_device(["vae"])
        condition_images = pipe.preprocess_video(condition_images)
        input_latents = pipe.vae.encode(condition_images, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
        # if pipe.scheduler.training:
        #     return {"latents": noise, "input_latents": input_latents}
        # else:
        #     latents = pipe.scheduler.add_noise(input_latents, noise, timestep=pipe.scheduler.timesteps[0])
        #     return {"latents": latents}
        return {"condition_emb": input_latents}
    ...
    **kwargs,
    ...
    if dit.seperated_timestep and fuse_vae_embedding_in_latents:
        timestep = torch.concat([
            ...
            #torch.zeros((latents.shape[2], latents.shape[3] * latents.shape[4] // 4), dtype=latents.dtype, device=latents.device)
        ]).flatten()
    ...
    if condition_emb is not None:
        condition_emb = condition_emb.permute(0, 2, 3, 4, 1)
        condition_emb = x+ dit.condition_proj(condition_emb)
        x = x + condition_emb
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            if use_gradient_checkpointing_offload:
                with torch.autograd.graph.save_on_cpu():
                    x = torch.utils.checkpoint.checkpoint(
                        ...
                        x, context, union_mask, first_masks, t_mod, freqs,
                        use_reentrant=False,
                    )
                ...
                x = block(x, context, union_mask, first_masks, t_mod,  freqs)
            ...
            if block_id == 27 and file_name != "" and check:
                attn_weights = block(x, context, union_mask, first_masks, t_mod, freqs, check=True)
                return attn_weights
