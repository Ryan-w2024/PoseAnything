...
"""bash
    python grounded_sam2_florence2_image_demo.py     --pipeline object_detection_segmentation
"""
...
def run_florence2(task_prompt, text_input, model, processor, image): #<OD> NULL 
...
def object_detection_and_segmentation(
    ...
    """
    largest_bbox, largest_label = get_largest_bbox(results)

    if largest_bbox is not None and largest_label is not None:
        results['<OD>']['bboxes'] = [largest_bbox]
        results['<OD>']['labels'] = [largest_label]
    else:
    ...
    if masks.ndim == 4:
        masks = masks.squeeze(1) #remove target dimension
    ...
    img = cv2.imread(image_path) #read in original picture
    file_name = os.path.splitext(os.path.basename(image_path))[0]
    masked_img_path = f"{file_name}.jpg"
    """
    ...
    if masks.ndim == 3:
        ...
        mask = masks.astype(bool)
    mask_3d = np.stack([mask] * 3, axis=-1) #extend to RGB
    masked_img = np.where(mask_3d, img, 255)
    cv2.imwrite(os.path.join(output_dir, masked_img_path), masked_img)
...
def open_vocabulary_detection_and_segmentation(
    ...
    label_annotator = sv.LabelAnnotator()
    annotated_frame = label_annotator.annotate(scene=annotated_frame, detections=detections, labels=labels)
    cv2.imwrite(os.path.join(output_dir, "test2_detection.jpg"), annotated_frame)
    
    mask_annotator = sv.MaskAnnotator()
    annotated_frame = mask_annotator.annotate(scene=annotated_frame, detections=detections)
    cv2.imwrite(os.path.join(output_dir, "test2_with_mask.jpg"), annotated_frame)
