...
def run_demo_video(args,video_path,tmp_root):
    args.visual_type = "branches" # "lines&pts" # 

    # step 1: video to images
    cap = cv2.VideoCapture()
    vfileid = video_path.split('.')[0]
    ...
    first_frame = cv2.imread(fnames[0])
    height, width, _ = first_frame.shape

    fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # MP4格式
    fps = 30.0  # 帧率
    output_path = f"{str(save_tmp_dir)}.mp4"
...
if __name__ == '__main__':
    ...
    video_path=f""
    tmp_root=f""
    run_demo_video(args,video_path,tmp_root)
